...
简单来说，这个项目能让你直接克隆任何一个优选好的域名（比如别人花时间筛选的 CDN 加速域名），并把它所有的 DNS 解析记录实时同步到你自己的域名上。
...
✨ 主要功能
功能点	图标	描述
一键域名克隆 (核心)	🔄	深度克隆 CNAME 记录找到最终 IP，并实时同步源站变化，实现全自动“白嫖”。
聚合 IP 到 GitHub	📦	自动化从多个公开源抓取最新 IP 列表，并自动推送到你自己的 GitHub 仓库。
订阅器友好 API	📡	直接访问同步到 GitHub 的 IP 文件。格式为 你的 Worker 首页 URL / GitHub 文件路径，可直接用于各种订阅器。
...
登录 Cloudflare 控制台，进入左侧菜单的 Workers & Pages。

点击 创建应用程序 (Create Application) > 创建 Worker (Create Worker)。

为你的 Worker 取一个名字（例如 cf-dns-clon），然后点击 部署 (Deploy)。

部署成功后，点击 编辑代码 (Edit code)。

将本项目提供的 src/index.js 文件中的所有代码，完整地复制并粘贴到代码编辑器中，覆盖掉原有的示例代码。
...
点击 创建数据库 (Create database)，填写数据库名称（例如 wuya-db），然后点击 创建 (Create)。

返回到你的 Worker，进入 设置 (Settings) > 变量 (Variables)。

找到 D1 数据库绑定 (D1 Database Bindings)，点击 添加绑定 (Add binding)。

变量名称 (Variable name) 必须填写 WUYA (全大写)。

在 D1 数据库 (D1 Database) 下拉列表中，选择你刚刚创建的 wuya-db。
...
登录 GitHub，点击 Generate new token (classic)。
...
返回到你的 Worker，进入 设置 (Settings) > 触发器 (Triggers)。

在 Cron 触发器 (Cron Triggers) 部分，点击 添加 Cron 触发器 (Add Cron Trigger)。

Cron 表达式 选择 每分钟 (* * * * *)，以确保同步的及时性。

点击 添加触发器 (Add trigger) 保存。
